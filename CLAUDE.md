...
- `.claude/settings.local.json`, `.claude/output-styles/`, `.claude/skills/`,
  `.agents/skills/`, `skills-lock.json` — per-machine tool state. The two
  skills trees were **byte-for-byte duplicates**, so the repo carried two
  copies of 28 vendored third-party files (shadcn, frontend-design and four
  others, one shipping its own `LICENSE.txt`) that are nobody's product code.
  Nothing in `core/`, `api/`, `web/`, `tests/` or `docs/` referenced them.
- `BUILD_PROMPTS.md` — the original phase-by-phase build script. This one is
  worth the extra sentence, because it looked like history and was not. It is
  a *forward-looking instruction sheet* ("feed these to Claude Code one at a
  time, in order"), which makes it the same artifact class as `TASKS.md`; and
  unlike a stale task list it has gone actively wrong. It specifies
  `RecipeIngredient` without `process_key`, a hand-declared
  `process_uncertainty` dict, and an eligibility filter gating on
  `process_uncertainty` alone. The first is the field D6's per-line
  attribution rests on; the second is what the round-4 addendum forbids
  ("never hand-computed once and pasted in"); the third is `docs/audit_log.md`
  finding 1, which this file corrects by name. A reader following it would
  build three defects this repo has already fixed, carrying the repo's own
  authority. `docs/build_log.md` is the durable record of what was built.
...
("Task T3 in `TASKS.md`"), and seven places in `core/` and `tests/` cite
`BUILD_PROMPTS.md` the same way (`core/nutrition/citations.py`,
`core/planner/{combinations,solver,validator}.py`,
`tests/test_planner_combinations.py`, `DESIGN_SYSTEM.md`). Those citations now
point outside the repo. They are left as-is — they date a decision rather than
direct a reader, and rewriting history to hide a working file would be worse
than a dangling reference. Do not add new ones: cite the `docs/` artifact
instead.

One of those was checked rather than assumed before removing: `citations.py`'s
...
planner modules, `core/foods/nutrition_of.py` (added 2026-08-09 for D6; the
harness never cared which package a module was in, so a new one costs a module
constant and an `OWN_TESTS` row) and `tests/conftest.py` (added 2026-08-09 for
D8 — the rule deciding what a skipped browser check *means* is a gate in this
sense, and the harness already copies `tests/`, so it is not a special case
either) and `core/foods/recipe_loader.py` (added 2026-08-09 for D10, rows
R1–R5) — one entry per mechanism and the smallest edit that removes it,
applied in a throwaway worktree. It copies `core/` and `tests/` from the
**working tree**, so a test can be graded before it is committed — grading only
committed code would mean believing a test before watching it fail, which is the
order this practice exists to reverse. Extend it when adding a gate; a new
mechanism without a row is a mechanism nobody will check again. A mechanism you
delete deliberately loses its row, and one you keep as documented-dead keeps it
with "survives" as the expected answer — a known answer stops the next sweep
reporting it as news.

**It cannot grade `web/`, and adding `web` to the copied trees would not fix
that.** The harness mutates a copy inside a throwaway worktree, but the browser
loads `web/` from a static server pointed at the real directory, so a mutated
copy has no effect and every row would falsely report "survived" — the most
dangerous answer this harness can give. Web mechanisms are deletion-checked by
hand against the working tree, with the transcript in `docs/audit_log.md`
(D9b, 2026-08-09, is the worked example). Do not add `web/` rows.
...
- **A test in the right file is not automatically the right test.** A test that
  ...
  reports red without protecting anything specific. Useful; not coverage. The
  harness compounds this: a `covered` row names the *first* scoped failure,
  which is collection order, not relevance. On D6 three of five rows named a
  test that had been explicitly demoted as not-evidence, while the mechanism's
  own test was also red and further down the list. Take the full failure list
  per mutation before believing the row, and check the reverse too — the D6
  entry's sharpest result is that the demoted test does not go red on either of
  the two defects the fix was actually about.
- **Check reachability before calling a survivor a hole.** See the standing list
  below.
- **A harness that parses tool output is itself a measurement.** The sweep read
  pytest's summary with `line.startswith("FAILED ")`, which matches nothing once
  pytest colours its output — and whether it colours depends on inherited
  `FORCE_COLOR`/`PY_COLORS`, i.e. on which shell launched the run. Identical code
  scored differently in two shells (`docs/audit_log.md` finding 35). Strip
  formatting before matching, and force the flag off rather than assuming a pipe
  means plain text.
- **An isolated run must isolate the whole input, not just the code.** The sweep
  copied `core/` and `tests/` from the working tree and left `data/` at HEAD, so
  D10's new loader met the old recipe YAML, every edited recipe was rejected on
  load and the shared `library` fixture errored on every run — mutation or not.
  It reported "5 covered" for five mechanisms it never exercised
  (`docs/audit_log.md` finding 42). Whatever a test reads is part of the tree
  under test.
...
FOODAI_WEB_TESTS=required python -m pytest tests/ -q
                                    # ...and this one before trusting a green
                                    #   run to mean the frontend was checked.
                                    #   Without it the browser tests skip when
                                    #   the servers are down and the run still
                                    #   exits 0 — announced since D8, but still
                                    #   exit 0. See tests/conftest.py.
