...
planner modules and `core/foods/nutrition_of.py` (added 2026-08-09 for D6; the
harness never cared which package a module was in, so a new one costs a module
constant and an `OWN_TESTS` row) — one entry per mechanism and the smallest edit that removes it,
