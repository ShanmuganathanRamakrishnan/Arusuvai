...
| `web/`                    | Partial — static landing page only (`web/index.html/styles.css/app.js`, a documented non-Next.js deviation; see `web/README.md`). No onboarding page yet: the onboarding increment stopped at `api/` per the plan, since a real page consumes the API rather than computing anything. |
