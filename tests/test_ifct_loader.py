...
class TestFixtureSet:
    def test_every_fixture_row_loads(self, load_report):
        ...
        assert len(load_report.loaded) == 30

    def test_no_ifct_code_is_invented(self, ingredients):
        # Five rows now carry real IFCT 2017 codes, extracted from a
        ...
        # same digitization (see data/raw/ifct/README.md for why). Every
        # other row is still hand-entered, and an invented-but-plausible code
        # would be worse than an absent one: it passes every check while
        # being wrong. So real codes are limited to exactly this known set;
    ...
    def test_unverified_rows_are_reported_not_silently_accepted(self, load_report):
        # 29 of 30 rows are unverified; only `water` (which has no nutrients to
        ...
        # source_note states. The five real-IFCT-code rows above (including
        # 2026-08-16's egg_boiled) are NOT exempted: their values were
        # extracted by this build, not opened by a human against the primary
        # source, so they stay verified=false pending that review (see
        # CLAUDE.md, "only a human... may flip that flag").
        assert len(load_report.warnings) == 29
